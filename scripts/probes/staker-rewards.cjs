...
(async () => {
  ...
  const rows = await Promise.all(ids.map(async (id) => {
    ...
    const [o, e22, e23, idx] = await Promise.all([r(POOLS, 'ownerOf', [id]), r(SPR, 'pendingStakerReward', [id, 22n]), r(SPR, 'pendingStakerReward', [id, 23n]), r(SPR, 'pendingIndexedStakerReward', [id])]);
    return { id, agent: p[1], amt: F(p[2]), owner: o, e22: F(e22), e23: F(e23), idx: F(idx) };
  }));
  let t22 = 0;
  for (const x of rows) { if (x.e22 !== 'ERR') t22 += Number(x.e22); console.log(`#${x.id}`.padEnd(4), String(x.agent).padEnd(6), name(x.agent).slice(0, 20).padEnd(20), String(x.amt).padStart(9), x.owner.slice(0, 10), 'e22', String(x.e22).padStart(9), 'e23', String(x.e23).padStart(9), 'indexed', x.idx); }
  console.log('sum e22 pending', t22.toFixed(1));
})();
